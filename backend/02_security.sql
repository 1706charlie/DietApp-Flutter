...
values (1, 'admin@gmail.com', '', 'Admin', 'admin', null),
       (2, 'svandenbussche@gmail.com', '', 'Sylvie', 'basic_user', 'BE45 4567 4567 4567');
...
select login('svandenbussche@gmail.com', 'Password1,'); -- OK
...
select auth.login_for_test('svandenbussche@gmail.com');
select current_user, auth.email(), auth.role(), auth.id();
select auth.logout_for_test();

select auth.login_for_test('admin@gmail.com');
select current_user, auth.email(), auth.role(), auth.id();
select auth.logout_for_test();
